            ...
            <Flex w="full" overflow="hidden" pos="relative">
                <Flex w="full" {...carouselStyle}>
                    {slides.map((slide, sid) => (
                        <Box key={`slide-${sid}`} boxSize="full" shadow="md" flex="none" p={[3, 3, 3, 0]}>
                            <Image
                                src={slide.img}
                                boxSize="full"
                                backgroundSize="cover"
                                alt={`Tockler screenshot ${sid}`}
                            />
                        </Box>
                    ))}
                </Flex>
                ...
                <Text {...arrowStyles} right="0" onClick={nextSlide} position="absolute">
                    &#10095;
                </Text>
                <HStack justify="center" pos="absolute" bottom="8px" w="full">
                    {Array.from({ length: slidesCount }).map((_, slide) => (
                        <Box
                            ...
                            m="0 2px"
                            bg={currentSlide === slide ? `primary` : `${dotColorMode}.500`}
                            rounded="50%"
